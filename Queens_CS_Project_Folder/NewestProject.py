...
sound_symbol = pygame.transform.scale(sound_symbol, (40, 40))
no_sound_symbol = pygame.image.load(PROJECT_ROOT / "Queens_CS_Project_Folder/speaker-slash.webp")
no_sound_symbol = pygame.transform.scale(no_sound_symbol, (40, 40))
...
intermediate_screen_3 = pygame.image.load(PROJECT_ROOT / "Queens_CS_Project_Folder/intermediate_screen_3.png")
intermediate_screen_3 = pygame.transform.scale(intermediate_screen_3, (480, 450))
...
nav_font = pygame.font.Font(PROJECT_ROOT / "Queens_CS_Project_Folder/Fonts/ContrailOne-Regular.ttf", 35)
...
home_button = Button((255, 255, 255), 100, 25, 100, 50, 2)
about_us_button = Button((255, 255, 255), 300, 25, 140, 50, 2)
news_button = Button((255, 255, 255), 540, 25, 100, 50, 2)
services_button = Button((255, 255, 255), 740, 25, 130, 50, 2)
contact_us_button = Button((255, 255, 255), 970, 25, 180, 50, 2)
...
music_button = Button((0, 153, 0), 1680, 0, 120, 50, 2)
...
game_one_restart_button = Button((255,255,255), 1505, 770, 300, 62, 0)
...
mainmenu_shape_game_button = Button((0,0,0), 55,750, 150, 100, 0)
...
def draw_intermediate(): 
    ...
    third_game = font.render("COUNTING GAME", True, (0,0,0))
...
def draw_shape_game():
    ...
    if shape_music_clicked == True:
        pygame.draw.rect(win, (0, 153, 0), pygame.Rect(1680, 0, 120, 50))
        win.blit(sound_symbol, (1720, 5))
    else:
        pygame.draw.rect(win, (255, 0, 0), pygame.Rect(1680, 0, 120, 50))
        win.blit(no_sound_symbol, (1720, 5))
...
def draw_counting_game(): 
    ...
    win.blit(medium_font.render(str(counting_game_score), True, (0,0,0)), (1700, 60))
...
while run: 
    for event in pygame.event.get(): 
        ...
        # if user presses key on keyboard
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            # if the user clicks Start, go into game selection screen
            ...
            # back to game selection screen from shape game
            ...
            # back to game selection screen from math game
            ...
            # text screen for inputting username (create account page)
            ...
            # text screen for inputting password (create account page)      
            ...
            # text screen for inputting username (login page)
            ...
            # text screen for inputting password (login page)                 
            ...
            # submit username + password for create account page
            ...
            # submit username + password for login page
            ...
            if counting_game == True and music_button.check_mouse_position(mouse_position):
                ...
                if not thirdgame_music_clicked:
                    ...
                    mixer.music.load(PROJECT_ROOT / "Queens_CS_Project_Folder/Concerto-no.-21-in-C-major-K.-467-II.-Andante.mp3")
                    mixer.music.play()
                    
            
            # if ok button clicked, start the game
            ...
            for number_points in number_point_list:
                if number_points.check_mouse_position(mouse_position) and game == True and counter_starter_button_boolean == False and start_first_game == True:
                    ...
                    threshold_counter += 1
                    # If capacity reached then a new threshold will be created, aka the next number (i.e. if user clicks on two then threshold will increase to three cause that is the next number)
            ...
            # for shape game: select shape to draw
            ...
            if shape_game_itself == True and back_button.check_mouse_position(mouse_position):
                fade()
                shape_game_itself = False
                shape_game = True
                    
            
            # save progress for shape game
            ...
            # back to main menu from the shape game
            if calculations_screen == True and mainmenu_shape_game_button.check_mouse_position(mouse_position): 
                calculations_screen = False
                shape_game_number_list = []
                calculator_list = []
                calculator_list_2 = []
            ...
            # back to game screen from shape game
    ...
    # display the appropriate stuff for each screen the user is on
    if start_game == True:  
        draw_start()
    ...
    if calculations_screen == True:
        ...
        mainmenu_shape_game_button.draw_button()
        ...
        # accracy for each point in the shape game
